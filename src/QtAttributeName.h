#pragma once

#include <QString>

#include "QtPropertyConfig.h"

namespace QtAttributeName {
    QTPROPERTYSHEET_DLL extern const QString Size;
    QTPROPERTYSHEET_DLL extern const QString MinValue;
    QTPROPERTYSHEET_DLL extern const QString MaxValue;
    QTPROPERTYSHEET_DLL extern const QString EnumName;
    QTPROPERTYSHEET_DLL extern const QString EnumValues;
    QTPROPERTYSHEET_DLL extern const QString FlagName;
    QTPROPERTYSHEET_DLL extern const QString Decimals;
    QTPROPERTYSHEET_DLL extern const QString ReadOnly;
    QTPROPERTYSHEET_DLL extern const QString FileDialogType;
    QTPROPERTYSHEET_DLL extern const QString FileDialogFilter;
    QTPROPERTYSHEET_DLL extern const QString FileRelativePath;
}
